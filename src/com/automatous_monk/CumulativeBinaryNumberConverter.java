 ...
 * @author Paul Reiners
 */
public class CumulativeBinaryNumberConverter extends BinaryNumberConverter {

    public CumulativeBinaryNumberConverter() {
        rowToNoteConverter = new CumBinNumToNoteConverter();
    }
    ...
    /* (non-Javadoc)
     * @see camusic.BinaryNumberConverter#convertCAHistoryToPhrase(camusic.InfiniteCellularAutomaton, camusic.HorizontalBias)
     */
    public Phrase convertCAHistoryToPhrase(
        CellularAutomaton cA,
        HorizontalBias bias,
        ...
        int[][] generations = cA.getGenerations();
        for (int i = 0; i < cA.getGenerationCnt(); i++) {
            int[] row = generations[i];
